...
class ModeFinderV2RawFinalRefine(ModeFinderV2):
    """
    Обычный v2, но финальный refinement при t=0 выполняется
    по ненормализованному score.

    Основной поиск, ascent и split остаются такими же, как в v2:
    normalize_score=True.
    """

    def _gradient_ascent_custom(
        self,
        x: np.ndarray,
        t: int,
        alpha: float,
    ...
    def _run_single_start(self, x0: np.ndarray, verbose: bool) -> List[np.ndarray]:
        ...
        for t in self.timesteps:
            ...
            # Основной поиск остается как в v2: normalized score.
            after_ascent = [
        ...
        # Главное отличие: финальный refinement без normalize_score.
        finals = [
            self._gradient_ascent_custom(
                x=x,
                t=0,
                alpha=alpha_refine,
...
def run_v2_raw_refine(model, true_modes, cfg, seed: int, eps_values: List[float]) -> Dict[str, Any]:
    mf = cfg.mode_finder_v2
    x_min, x_max = cfg.distribution.x_min, cfg.distribution.x_max

    finder = ModeFinderV2RawFinalRefine(
        model=model,
...
def main():
    ...
    parser.add_argument("--eps", default="1.0,2.0")
    parser.add_argument("--checkpoint-dir", default="./checkpoints")
    parser.add_argument("--output-dir", default="./results_10d/v2_raw_refine_k7")
    ...
    logger.info(
        "V2 RAW FINAL REFINE TEST: dim=%d K=%d n_seeds=%d n_init_seeds=%d eps=%s",
    ...
    for dist_seed in range(args.n_seeds):
        ...
        for init_seed in range(args.n_init_seeds):
            logger.info(
                "RUN v2_raw_refine K=%d dseed=%d init_seed=%d",
                args.K, dist_seed, init_seed,
            )

            run = run_v2_raw_refine(
                model=model,
            ...
            logger.info(
                "DONE v2_raw_refine K=%d dseed=%d init_seed=%d nfe=%d modes=%d | %s",
                args.K,
                dist_seed,
                init_seed,
                run["nfe"],
    ...
    out_path = out_dir / f"v2_raw_refine_K{args.K}_dim{cfg.dim}.json"
    ...
    payload = {
        "config": {
            ...
            "eps_values": eps_values,
            "method": "v2_raw_final_refine",
            "note": "Main v2 search uses normalize_score=True; final t=0 refinement uses raw unnormalized score.",
        },
        "runs": all_runs,
        "summary": summary,
    }
